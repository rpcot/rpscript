...
import ru.rpcot.parserjava.parser.expressions.*;
import ru.rpcot.parserjava.parser.statements.*;
...
public class Parser {
    ...
    private double[] getBp(TokenType type) {
        return switch (type) {
            case EQUALS -> new double[]{0.5, 0.6};
    ...
    private Expression parseExpression(double bp) {
        ...
        switch (token.getType()) {
            case IDENTIFIER, NUMBER, BOOLEAN, STRING -> consume(token.getType());
            case LPAR -> {
        ...
        while (true) {
            ...
            if (op.getType() == TokenType.LPAR) {
                ...
                lhs = new BinaryExpression(lhs, op.getType(), rhs);
            }
        }
        return lhs;
    }
